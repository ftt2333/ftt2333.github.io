...
// so adding an empty div and check it when decrption.
  ...
  var original_content = document.getElementById('hexo-blog-encrypt').innerHTML
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      hideButton.textContent = 'Relock';
      hideButton.type = 'button';
      hideButton.classList.add("ui");
      hideButton.classList.add("button");
      hideButton.classList.add("hbe-button-re");
      hideButton.addEventListener('click', () => {
        window.localStorage.removeItem(storageName);
        document.getElementById('hexo-blog-encrypt').innerHTML = original_content;
        var event = new Event('hexo-blog-encrypt');
  ...
  function hbeLoader() {
    ...
    if (!mainElement.added_listener) {
      mainElement.added_listener = true
...
hbe_init()
$(document).on('pjax:complete', hbe_init)